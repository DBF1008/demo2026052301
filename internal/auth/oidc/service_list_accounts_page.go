...
// ListAccountsPage lists up to page size oidc accounts, filtering out entries that
// do not pass the filter item function. It will automatically request
// more oidc accounts from the database, at page size chunks, to fill the page.
...
func ListAccountsPage(
	...
	const op = "oidc.ListAccountsPage"
	...
	listItemsFn := func(ctx context.Context, lastPageItem auth.Account, limit int) ([]auth.Account, time.Time, error) {
		...
		oidcAccounts, listTime, err := repo.listAccounts(ctx, authMethodId, opts...)
		if err != nil {
			return nil, time.Time{}, err
		}
		var accts []auth.Account
		for _, acct := range oidcAccounts {
