...
// ListManagedGroupsRefresh lists oidc managed groups according to the page size
...
func ListManagedGroupsRefresh(
	...
	const op = "oidc.ListManagedGroupsRefresh"
	...
	listItemsFn := func(ctx context.Context, lastPageItem auth.ManagedGroup, limit int) ([]auth.ManagedGroup, time.Time, error) {
		...
		oidcManagedGroups, listTime, err := repo.ListManagedGroupsRefresh(ctx, authMethodId, rt.PreviousPhaseUpperBound.Add(-globals.RefreshReadLookbackDuration), opts...)
		if err != nil {
