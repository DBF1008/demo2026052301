...
// ListAccounts lists up to page size oidc accounts, filtering out entries that
...
func ListAccounts(
	...
	listItemsFn := func(ctx context.Context, lastPageItem auth.Account, limit int) ([]auth.Account, time.Time, error) {
		...
		oidcAccts, listTime, err := repo.listAccounts(ctx, authMethodId, opts...)
		if err != nil {
