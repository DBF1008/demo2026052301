...
resource "enos_local_exec" "wait_for_ssh" {
  ...
  depends_on = [time_sleep.wait_5_minutes]
  inline     = ["timeout 900s bash -c 'until ssh -i ${local.private_key} -o BatchMode=Yes -o IdentitiesOnly=yes -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -o ConnectTimeout=5 Administrator@${aws_instance.member_server.public_ip} \"echo ready\"; do sleep 10; done'"]
}
